...
-- wall
----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wall:Open")
AddEventHandler("wall:Open",function(source)
    if not admin then
		return
	end
						...
						if admin and (PlayerPedId() ~= GetPlayerPed(id)) then
							if GetPlayerPed(id) ~= -1 and players[id] ~= nil then
								...
								if distance <= Config.Distanciaesp then
									HazeStore = 1
									DrawText3D(x2, y2, z2+1.0,"~w~[~g~"..distance.." m~w~]~w~\n[ ~b~"..players[id].."~w~ ] "..name.." [Vida: ~g~"..vidaPorcento.."~w~]\n~w~Arma [~p~"..arma.."~w~] Colete [~b~"..colete.."~w~]")
									--DrawText3D(x2, y2, z21.5,"~b~"..arma.."")
								end
							end
						end
					end
					...
					if ((NetworkIsPlayerActive( id )) and GetPlayerPed(id) ~= PlayerPedId()) then	
						if admin and (PlayerPedId() ~= GetPlayerPed(id)) then
							if GetPlayerPed(id) ~= -1 and players[id] ~= nil then
								...
								if distance <= Config.Distanciaesp then
									HazeStore = 1
									DrawLine(gete, entityc, cordes, x, y, z, 0, 0, 255, 255)
								end
