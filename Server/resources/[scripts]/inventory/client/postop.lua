		...
		if InService then
			if not IsPedInAnyVehicle(Ped) then
				...
				if Distance <= 30 then
					...
					DrawText3D(DeliveryCoords[Packages][1], DeliveryCoords[Packages][2], DeliveryCoords[Packages][3],"~g~E~w~   ENTREGAR ENCOMENDA",350,0.0125,0.03)
					if Distance <= 0.75 and IsControlJustPressed(1,38) and HandPackage then
						InPackage = false
						HandPackage = false
