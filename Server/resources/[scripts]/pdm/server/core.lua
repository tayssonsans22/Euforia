		...
		if not Active[Passport] then
			...
			if Vehicle[1] then
				...
				elseif VehicleMode(Name) == "Work" then
					if vRP.Request(source, "Concessionária", "Comprar <b>" .. VehicleName(Name) .. "</b> por <b>$" .. parseFormat(VehiclePrice) .. "</b> reais?") then
	...
	if Passport then
		if not Active[Passport] then
			...
			if vRP.HasGroup(Passport, "Premium") then
				...
				if vRP.Request(source, "Concessionária", "Iniciar o teste por <b>$100</b> reais?") then
