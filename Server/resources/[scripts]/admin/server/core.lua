...
AddEventHandler("admin:Dynamic", function(Mode)
	...
	if Passport then
		...
		elseif Mode == "blips" then
			if vRP.HasGroup(Passport,"Admin",2) then
				TriggerClientEvent("wall:Open",source)
			else
				TriggerClientEvent("Notify",source,"amarelo","Você não tem permissões para isso.","Atenção",5000)
			end
		...
		elseif Mode == "rembank" then
			if vRP.HasGroup(Passport,"Admin") then
				...
				if Keyboard then
					...
					TriggerClientEvent("NotifyItens",source,{ "-", "dollars", parseFormat(Keyboard[2]), "reais" })
				end
			else
				TriggerClientEvent("Notify",source,"amarelo","Você não tem permissões para isso.","Atenção",5000)
			end
		...
		elseif Mode == "statsPolicia" then
			local TotalPolicia = vRP.NumPermission("Policia")
		...
		elseif Mode == "statsParamedico" then
			local TotalParamedico = vRP.NumPermission("Paramedico")
...
-- RegisterCommand("teste",function(source)
	...
	-- if Passport then
		-- if vRP.HasGroup(Passport,"Admin") then
			-- TriggerClientEvent("Notify",source,"","",false,5000)
		-- end
	-- end
-- end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- ColocarIdNoApelido = function(source)
-- 	local source = source 
-- 	local Passport = vRP.Passport(source)
...
-- 	local link = "https://discord.com/api/webhooks/1170547751428292619/diZAnQnoFlLyXHt85TxTZbrlDspfD3mjKDS8fc1AVuwioPdLLAOEpJyeq55hkCcKj_Ms"
...
-- end
-- 
-- RegisterCommand("Puxar",function(source)
-- 	local source = source 
-- 	ColocarIdNoApelido(source)
-- end)
...
-- RegisterNetEvent("dumonster:ApagarCarros"),
-- AddEventHandler("dumonster:ApagarCarros",function(source,args)
-- 	local source = source

-- 	-- Definir valor para Network - > Solicitar o return da funçao abaixo
...
-- -- Colocar no client para testar
-- function Hensa.ClosestVehicle(Source)
-- 	local Selected = 0
-- 	local Ped = PlayerPedId()
-- 	local Radius = 10000 + 0.0001
-- 	--local Coords = GetEntityCoords(Ped)
