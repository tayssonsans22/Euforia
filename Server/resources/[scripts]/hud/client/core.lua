		...
		if LocalPlayer["state"]["Active"] then
			...
			if IsPauseMenuActive() then
				...
				if Display then
					...
					SendNUIMessage({ Action = "Weather", Weather = ClassWeather(GlobalState["Weather"]) })
			...
			if GetEntityHealth(Ped) > 100 then
				...
				if Oxigen <= 0 and OxigenTimer <= GetGameTimer() then
					OxigenTimer = GetGameTimer() + 30000
