	...
	if Passport then
		if vRP.GetHealth(source) <= 100 then
			...
			if vRP.TakeItem(Passport, "paramedicpass", 1, true) then
				...
				return true
			elseif vRP.UserMedicPlan(Passport) then
				if vRP.Request(source, "Hospital", "Prosseguir o tratamento por <b>$"..parseInt(Price * 0.50).."</b> "..itemName("dollars").."?") then
		...
		if os.time() >= Announces[Service] then
			if Service == "Policia" then
				if vRP.HasGroup(Passport, "Policia") then
					...
					TriggerClientEvent("Notify", -1, "policia", "Sinta-se seguro(a) agora mesmo, pois há policiais realizando patrulhas em nossa cidade neste exato instante.", "PMRJ", 15000)
				else
			...
			elseif Service == "Paramedico" then
				if vRP.HasGroup(Passport, "Paramedico") then
					Announces[Service] = os.time() + 600
					TriggerClientEvent("Notify", -1, "hospital", "Estamos em busca de doadores de sangue, seja solidário e ajude o próximo, procure um de nossos profissionais.", "UPA 24h", 15000)
				else
					TriggerClientEvent("Notify", source, "vermelho", "Você não pode enviar um anúncio.", "Aviso", 5000)
				end
			elseif Service == "Mecanico" then
				if vRP.HasGroup(Passport, "Mecanico") then
					Announces[Service] = os.time() + 600
					TriggerClientEvent("Notify", -1, "mecanico", "Necessita dos serviços de uma oficina mecânica? Temos profissionais disponíveis. Faça agora mesmo a sua solicitação.", "Mecânica", 15000)
				else
					TriggerClientEvent("Notify", source, "vermelho", "Você não pode enviar um anúncio.", "Aviso", 5000)
				end
			else
...
AddEventHandler("target:Medicplan", function()
	...
	local Passport = vRP.Passport(source)
	if Passport then
		if vRP.Request(source, "Hospital", "Assinar o plano de saúde por <b>$"..parseInt(10000).."</b> "..itemName("dollars").."? Lembrando que a duração do mesmo é de 7 dias.") then
			if vRP.PaymentFull(Passport, 10000) then
				vRP.SetMedicPlan(Passport)
				TriggerClientEvent("Notify", source, "azul", "Você assinou o plano de saúde por <b>7 dias</b>.", "Pillbox Medical", 5000)
			end
		end
	end
end)