...
AddEventHandler("player:Charge", function(Entity)
	...
	if Passport then
		...
		if Identity then
			...
			if Keyboard then
				if vRP.Request(Entity, "Cobrança", "Aceitar a cobrança de <b>$" .. parseFormat(Keyboard[1]) .. "</b> feita por <b>"..vRP.FullName(Passport).."</b>?") then
					if vRP.GetBank(Entity) >= Keyboard[1] then
						TriggerClientEvent("NotifyItens", Entity, { "-", "dollars", parseFormat(Keyboard[1]), "reais" })
...
AddEventHandler("player:GetCard",function()
	...
	if Passport then
		if vRP.MaxItens(Passport,"bankcard",1) then
			...
			if vRP.Request(source,"Banco","Deseja emitir <b>1x "..itemName("bankcard").."</b> por <b>$"..parseFormat(CardPrice).."</b> reais?") then
...
function Hensa.PayWash()
	...
	local Passport = vRP.Passport(source)
	if Passport then
		if vRP.Request(source,"Car Wash","A limpeza do veículo custa <b>$"..parseFormat(WashPrice).."</b> reais, deseja prosseguir?") then
