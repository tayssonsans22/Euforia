...
function Hensa.Verify(Number)
	...
	if Passport then
		...
		-- 		TriggerClientEvent("Notify", source, "amarelo", "Você não possuí <b>Carteira de Habilitação</b>.", "Atenção", 5000)
		-- 		return false
...
function Hensa.ServerVehicle(Model, x, y, z, Heading, Plate, Nitrox, Doors, Body, Fuel)
	...
	while not DoesEntityExist(Vehicle) and SpawnVehicle <= 1000 do
...
AddEventHandler("garages:Impound", function(vehName)
	...
	if Passport then
		...
		if vRP.Request(source, "Garagem", "A liberação do veículo tem o custo de <b>$" .. parseFormat(VehiclePrice * 0.25) .. "</b> reais, deseja prosseguir com a liberação do mesmo?") then
...
function Hensa.Sell(Name)
	...
	if Passport then
		...
		local Consult = vRP.Query("vehicles/selectVehicles", { Passport = Passport, vehicle = Name })
		if Consult[1] then
			if not Consult[1]["mode"] == "dismantle" then
...
function Hensa.Transfer(Name)
	...
	if Passport then
		...
		if MyVehicle[1] then
			if not MyVehicle[1]["mode"] == "dismantle" then
		...
		if not vehicle[1] then
			if parseInt(Gemstone) > 0 then
				...
				if parseInt(VehiclePrice) > 0 then
					if vRP.Request(source, "Garagem", "Comprar <b>" .. VehicleName(Name) .. "</b> por <b>$" .. parseFormat(VehiclePrice) .. "</b> reais?") then
