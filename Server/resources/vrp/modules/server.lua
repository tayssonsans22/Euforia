...
AddEventHandler("Queue:Connecting", function(source, identifiers, deferrals)
	...
	if Identities then
		...
		if not vRP.Banned(Identities) then
			if Whitelisted then
				local Account = vRP.Account(Identities)
...
function vRP.ChosenCharacter(source, Passport, Model)
	...
	if not Characters[source] then
		...
		if #Consult > 0 then
			...
			Characters[source]["Medic"] = Consult[1]["Medic"]
...
function vRP.SetMedicPlan(source)
	if Characters[source] then
		vRP.Query("characters/SetMedicPlan",{ Medic = os.time() + 604800, Passport = Characters[source]["id"] })
...
function vRP.UpgradeMedicPlan(source)
	if Characters[source] then
		vRP.Query("characters/UpdateMedicplan",{ Passport = Characters[source]["id"] })
...
-- LOGS DEV
-----------------------------------------------------------------------------------------------------------------------------------------
AddEventHandler("onResourceStart",function(src)
	if src == "vrp" then 
		print("Base Iniciada com sucesso! Euforia está online com d%d% slots.")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- FOODS
